...
export const AddSubscriptionModal: React.FC<AddSubscriptionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to record subscription.')
    }
  }
        ...
        <div className="flex items-center justify-between border-b border-[#E0D8CE] pb-4">
          <div>
            ...
            <p className="font-mono-numbers text-xs text-[#6B6057]">
              Record a new recurring subscription ledger entry
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-[#E0D8CE] bg-[#F3EDE4] py-2 pl-9 pr-3 font-mono-numbers text-sm text-[#2A2420] placeholder-[#6B6057]/60 focus:border-[#2A2420] focus:outline-hidden"
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea
                rows={2}
                placeholder="Optional notes or plan category"
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                className="w-full rounded-lg border border-[#E0D8CE] bg-[#F3EDE4] py-2 pl-9 pr-3 text-sm text-[#2A2420] placeholder-[#6B6057]/60 focus:border-[#2A2420] focus:outline-hidden resize-none"
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center justify-end gap-2 border-t border-[#E0D8CE] pt-4">
            ...
            >
              ...
              <span>{isSubmitting ? 'Recording...' : 'Record Commitment'}</span>
            </button>
          </div>
        </form>
      </div>
